...
#define YT_LINUX_SUBVERSION 12574
#define YT_LINUX_VERSIONID "2.2.12574"
...
#define YTPHY_WOL_FEATURE_INTR_SEL_CFG                                         \
	BIT(6) /* WOL Event Interrupt Enable Config */
...
static int yt8521_probe(struct phy_device *phydev)
{
	...
	case 3: // utp<>sgmii
	case 0: // utp<>rgmii
		ytphy_write_ext(phydev, 0x100, 0); //set reg page
		//ytphy_write_ext(phydev, 0xa00d, 0x2600);
		//ytphy_write_ext(phydev, 0xa00e, 0x1800); //led status
...
/*
static int yt8521_xtal_init(struct phy_device *phydev)
{
	int ret = 0;
	int val = 0;
	// bool state = false;
	...
	msleep(50);
	ytphy_write_ext(phydev, 0xa012, 0x78);

	do {
		ret = ytphy_write_ext(phydev, 0xa012, 0x78);
		if (ret < 0)
			return ret;
...
static int yt8521_config_init(struct phy_device *phydev)
{
	...
	struct yt8xxx_priv *priv = phydev->priv;
	//yt8521_xtal_init(phydev); //set 125Mclk output for gmac

	ytphy_write_ext(phydev, 0xa00d, 0x2600); //led status
	ytphy_write_ext(phydev, 0xa00e, 0x1800);
	...
	netdev_info(
		phydev->attached_dev,
		"%s done, phy addr: %d, chip mode = %d, polling mode = %d\n",
		__func__, phydev->mdio.addr, priv->chip_mode,
		priv->polling_mode);
#endif
	return ret;
}
	...
	if (priv->polling_mode != YT_PHY_MODE_UTP) {
		...
		if (link && yt8521_fiber_latch_val != yt8521_fiber_curr_val) {
			...
			netdev_info(
				...
				__func__, phydev->addr, yt8521_fiber_latch_val,
...
static int yt8614_config_init(struct phy_device *phydev)
{
	...
	netdev_info(phydev->attached_dev,
		    "%s done, phy addr: %d, chip mode: %d, polling mode = %d\n",
		    __func__, phydev->addr, priv->chip_mode,
		    priv->polling_mode);
#else
	netdev_info(phydev->attached_dev,
		    "%s done, phy addr: %d, chip mode: %d, polling mode = %d\n",
	...
	if (priv->polling_mode & YT_PHY_MODE_FIBER) {
		...
		if (link && yt8614Q_fiber_latch_val != yt8614Q_fiber_curr_val) {
			...
			netdev_info(
				phydev->attached_dev,
				"%s, phy addr: %d, fiber link down detect, latch = %04x, curr = %04x\n",
				__func__, phydev->addr, yt8614Q_fiber_latch_val,
				yt8614Q_fiber_curr_val);
#else
			netdev_info(
				phydev->attached_dev,
				"%s, phy addr: %d, fiber link down detect, latch = %04x, curr = %04x\n",
				__func__, phydev->mdio.addr,
				yt8614Q_fiber_latch_val,
				yt8614Q_fiber_curr_val);
#endif
		}
...
static int yt8821_adjust_status(struct phy_device *phydev, int val)
{
	...
	speed_mode_bit15_14 =
		(val & YTXXXX_SPEED_MODE) >> YTXXXX_SPEED_MODE_BIT;
