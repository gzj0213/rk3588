 ...
 */
int tty_ldisc_receive_buf(struct tty_ldisc *ld, const unsigned char *p, char *f,
			  int count)
{
...
static int tty_rt_task(void *data)
{
	...
	set_task_comm(current, "uart2_rt_task");
...
void tty_flip_buffer_push(struct tty_port *port)
{
	...
	queue_work(system_unbound_wq, &buf->work);
	/*if (port->tty && port->tty->driver->subtype == SERIAL_TYPE_NORMAL &&
	    port->tty->index == 8) {
		if (port->uart_task) {
