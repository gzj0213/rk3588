	...
	for (i = 0; p && p->flags != 0; p++, i++) {
		...
		if (ret < 0) {
			dev_err(&dev->dev,
				...
				i, p->iobase, (unsigned long long)p->mapbase,
				p->irq, ret);
		}
	}
	return 0;
...
{
	struct uart_8250_port *up = container_of(
		to_delayed_work(work), struct uart_8250_port, overrun_backoff);
	struct uart_port *port = &up->port;
	unsigned long flags;
...
MODULE_PARM_DESC(nr_uarts,
		 "Maximum number of UARTs supported. (1-" __MODULE_STRING(
...
#ifdef CONFIG_SERIAL_8250_RSA
	__module_param_call(MODULE_PARAM_PREFIX, probe_rsa, &param_array_ops,
			    .arr = &__param_arr_probe_rsa, 0444, -1, 0);
#endif
}
#else
