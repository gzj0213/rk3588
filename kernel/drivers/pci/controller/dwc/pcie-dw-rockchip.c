					   ...
					   enum dw_pcie_as_type as_type)
{
	...
	rk_pcie_writel_ib_unroll(pci, index, PCIE_ATU_UNR_REGION_CTRL1,
...
static int rk_pcie_ep_inbound_atu(struct rk_pcie *rk_pcie, enum pci_barno bar,
				  dma_addr_t cpu_addr,
				  enum dw_pcie_as_type as_type)
{
	int ret;
...
static void rk_pcie_prog_outbound_atu(struct dw_pcie *pci, int index, int type,
...
static void __rk_pcie_ep_reset_bar(struct rk_pcie *rk_pcie, enum pci_barno bar,
				   int flags)
{
	u32 reg;
...
static void disable_aspm_l1ss(struct rk_pcie *rk_pcie)
{
	...
	val &= ~(PCI_L1SS_CAP_ASPM_L1_1 | PCI_L1SS_CAP_ASPM_L1_2 |
		 PCI_L1SS_CAP_L1_PM_SS);
	dw_pcie_writel_dbi(rk_pcie->pci, cfg_link_cap_l1sub, val);
}
#else
static inline void disable_aspm_l1ss(struct rk_pcie *rk_pcie)
{
	return;
}
#endif
...
static inline void rk_pcie_set_mode(struct rk_pcie *rk_pcie)
{
	...
	case RK_PCIE_RC_TYPE:
		if (rk_pcie->supports_clkreq) {
			...
			rk_pcie_writel_apb(rk_pcie, PCIE_CLIENT_POWER,
					   0x00010001);
		} else {
		...
		if (rk_pcie->is_rk1808)
			dw_pcie_writel_dbi(rk_pcie->pci,
					   PCIE_PL_ORDER_RULE_CTRL_OFF, 0xff00);
		break;
	}
}
...
static int rk_pcie_establish_link(struct dw_pcie *pci)
{
	...
	for (hw_retries = 0; hw_retries < RK_PCIE_ENUM_HW_RETRYIES;
	     hw_retries++) {
		/* Rest the device */
		...
		for (retries = 0; retries < 100; retries++) {
			...
			dev_info_ratelimited(
				pci->dev, "PCIe Linking... LTSSM is 0x%x\n",
				rk_pcie_readl_apb(rk_pcie,
						  PCIE_CLIENT_LTSSM_STATUS));
			rk_pcie_debug_dump(rk_pcie);
			msleep(20);
		...
		dev_err(pci->dev,
			"PCIe Link Fail, LTSSM is 0x%x, hw_retries=%d\n", ltssm,
			hw_retries);
...
{
	return dw_pcie_readl_dbi(rk_pcie->pci,
				 PCIE_DMA_OFFSET + PCIE_DMA_CTRL_OFF);
}
...
static int rk_pcie_ep_set_bar_flag(struct rk_pcie *rk_pcie,
...
static void rk_pcie_ep_setup(struct rk_pcie *rk_pcie)
{
	...
	if (!resbar_base) {
		...
		for (bar = 2; bar < 6; bar++) {
			dw_pcie_writel_dbi(rk_pcie->pci,
					   resbar_base + 0x4 + bar * 0x8,
					   0xfffff0);
			dw_pcie_writel_dbi(rk_pcie->pci,
					   resbar_base + 0x8 + bar * 0x8,
					   0x6c0);
		}

		/* Set flags */
		rk_pcie_ep_set_bar_flag(rk_pcie, BAR_0,
					PCI_BASE_ADDRESS_MEM_TYPE_32);
		rk_pcie_ep_set_bar_flag(rk_pcie, BAR_1,
					PCI_BASE_ADDRESS_MEM_TYPE_32);
		rk_pcie_ep_set_bar_flag(rk_pcie, BAR_2,
					PCI_BASE_ADDRESS_MEM_PREFETCH |
						PCI_BASE_ADDRESS_MEM_TYPE_64);
		rk_pcie_ep_set_bar_flag(rk_pcie, BAR_4,
					PCI_BASE_ADDRESS_MEM_PREFETCH |
						PCI_BASE_ADDRESS_MEM_TYPE_64);
	}
...
static int rk_pcie_ep_win_parse(struct rk_pcie *rk_pcie)
{
	...
	rk_pcie->ib_window_map =
		devm_kcalloc(dev, BITS_TO_LONGS(rk_pcie->num_ib_windows),
			     sizeof(long), GFP_KERNEL);
	if (!rk_pcie->ib_window_map)
		return -ENOMEM;

	rk_pcie->ob_window_map =
		devm_kcalloc(dev, BITS_TO_LONGS(rk_pcie->num_ob_windows),
			     sizeof(long), GFP_KERNEL);
			    ...
			    struct platform_device *pdev)
{
	...
	if (IS_ENABLED(CONFIG_PCI_MSI)) {
		...
		if (pp->msi_irq < 0) {
			...
			rk_pcie_host_ops.set_num_vectors =
				rk_pcie_msi_set_num_vectors;
		}
	}
...
static int rk_pcie_add_ep(struct rk_pcie *rk_pcie)
{
	...
	rk_pcie->pci->atu_base =
		rk_pcie->pci->dbi_base + DEFAULT_DBI_ATU_OFFSET;
	rk_pcie->pci->iatu_unroll_enabled =
		rk_pcie_iatu_unroll_enabled(rk_pcie->pci);
				...
				struct rk_pcie *rk_pcie)
{
	...
	rk_pcie->pci->dbi_base2 =
		rk_pcie->pci->dbi_base + PCIE_TYPE0_HDR_DBI2_OFFSET;

	apb_base =
		platform_get_resource_byname(pdev, IORESOURCE_MEM, "pcie-apb");
	if (!apb_base) {
	...
	rk_pcie->prsnt_gpio =
		devm_gpiod_get_optional(&pdev->dev, "prsnt", GPIOD_IN);
...
static int rk_pcie_phy_init(struct rk_pcie *rk_pcie)
{
	...
	if (ret) {
		dev_err(dev, "fail to set phy to  mode %s, err %d\n",
			(rk_pcie->phy_sub_mode == PHY_MODE_PCIE_RC) ? "RC" :
								      "EP",
			ret);
		return ret;
	}
...
static void rk_pcie_start_dma_rd(struct dma_trx_obj *obj, struct dma_table *cur,
				 int ctr_off)
{
...
static void rk_pcie_start_dma_wr(struct dma_trx_obj *obj, struct dma_table *cur,
				 int ctr_off)
{
	struct rk_pcie *rk_pcie = dev_get_drvdata(obj->dev);
...
{
	struct rk_pcie *rk_pcie =
		container_of(work, struct rk_pcie, hot_rst_work);
	u32 val, status;
	int ret;

	dev_err(rk_pcie->pci->dev, "%s!\n", __func__);
	...
	if (rk_pcie_readl_apb(rk_pcie, PCIE_CLIENT_HOT_RESET_CTRL) &
	    PCIE_LTSSM_APP_DLY2_EN) {
		ret = readl_poll_timeout(
			rk_pcie->apb_base + PCIE_CLIENT_LTSSM_STATUS, status,
			((status & 0x3F) == 0), 100, RK_PCIE_HOTRESET_TMOUT_US);
		if (ret)
			dev_err(rk_pcie->pci->dev,
				"wait for detect quiet failed!\n");

		rk_pcie_writel_apb(rk_pcie, PCIE_CLIENT_HOT_RESET_CTRL,
				   (PCIE_LTSSM_APP_DLY2_DONE) |
					   ((PCIE_LTSSM_APP_DLY2_DONE) << 16));
	}
}
...
static irqreturn_t rk_pcie_sys_irq_handler(int irq, void *arg)
{
	...
	status.asdword = dw_pcie_readl_dbi(
		rk_pcie->pci, PCIE_DMA_OFFSET + PCIE_DMA_WR_INT_STATUS);
	for (chn = 0; chn < PCIE_DMA_CHANEL_MAX_NUM; chn++) {
		if (status.donesta & BIT(chn)) {
			...
			if (rk_pcie->dma_obj && rk_pcie->dma_obj->cb)
				rk_pcie->dma_obj->cb(rk_pcie->dma_obj, chn,
						     DMA_TO_BUS);
		}
	...
	status.asdword = dw_pcie_readl_dbi(
		rk_pcie->pci, PCIE_DMA_OFFSET + PCIE_DMA_RD_INT_STATUS);
	for (chn = 0; chn < PCIE_DMA_CHANEL_MAX_NUM; chn++) {
		if (status.donesta & BIT(chn)) {
			clears.doneclr = 0x1 << chn;
			dw_pcie_writel_dbi(rk_pcie->pci,
...
static int rk1808_pcie_fixup(struct rk_pcie *rk_pcie, struct device_node *np)
{
	...
	rk_pcie->usb_pcie_grf =
		syscon_regmap_lookup_by_phandle(np, "rockchip,usbpciegrf");
...
{
	irq_set_chip_and_handler(irq, &rk_pcie_legacy_irq_chip,
				 handle_level_irq);
...
static int rk_pcie_init_irq_domain(struct rk_pcie *rockchip)
{
	...
	rockchip->irq_domain = irq_domain_add_linear(
		intc, PCI_NUM_INTX, &intx_domain_ops, rockchip);
...
{
	return single_open(file, rockchip_pcie_rasdes_show, inode->i_private);
}
...
static int rockchip_pcie_debugfs_init(struct rk_pcie *pcie)
{
	...
	debugfs_create_devm_seqfile(pcie->pci->dev, "dumpfifo", pcie->debugfs,
				    rockchip_pcie_fifo_show);
	file = debugfs_create_file("err_event", 0644, pcie->debugfs, pcie,
...
static int rk_pcie_really_probe(void *p)
{
	...
	rk_pcie->supports_clkreq =
		device_property_read_bool(dev, "supports-clkreq");
...
retry_regulator:
	...
	rk_pcie->hot_rst_wq =
		create_singlethread_workqueue("rk_pcie_hot_rst_wq");
	...
	if (!ret) {
		...
		if (irq >= 0) {
			irq_set_chained_handler_and_data(
				irq, rk_pcie_legacy_int_handler, rk_pcie);
			/* Unmask all legacy interrupt from INTA~INTD  */
			rk_pcie_writel_apb(rk_pcie,
	...
	if (!device_property_read_u32_array(dev, "rockchip,compliance-mode",
		...
		if (!rk_pcie->comp_prst[0]) {
			...
			dev_info(
				dev,
				"compliance mode for soldered board Gen%d, P%d.\n",
				rk_pcie->comp_prst[0], rk_pcie->comp_prst[1]);
			val = dw_pcie_readl_dbi(
				pci, PCIE_CAP_LINK_CONTROL2_LINK_STATUS);
			val |= BIT(4) | rk_pcie->comp_prst[0] |
			       (rk_pcie->comp_prst[1] << 12);
			dw_pcie_writel_dbi(
				pci, PCIE_CAP_LINK_CONTROL2_LINK_STATUS, val);
		}
		rk_pcie->is_signal_test = true;
	}
	...
	if (IS_ENABLED(CONFIG_DEBUG_FS)) {
		...
		val = dw_pcie_find_ext_capability(rk_pcie->pci,
						  PCI_EXT_CAP_ID_VNDR);
		if (!val) {
...
static void rk_pcie_downstream_dev_to_d0(struct rk_pcie *rk_pcie, bool enable)
{
	...
	if (enable) {
		if (rk_pcie->l1ss_ctl1)
			dw_pcie_writel_dbi(rk_pcie->pci,
					   bridge->l1ss + PCI_L1SS_CTL1,
					   rk_pcie->l1ss_ctl1);

		/* rk_pcie->aspm woule be saved in advance when enable is false */
		dw_pcie_writel_dbi(rk_pcie->pci,
				   bridge->pcie_cap + PCI_EXP_LNKCTL,
				   rk_pcie->aspm);
	} else {
		val = dw_pcie_readl_dbi(rk_pcie->pci,
					bridge->l1ss + PCI_L1SS_CTL1);
		...
		dw_pcie_writel_dbi(rk_pcie->pci,
				   bridge->pcie_cap + PCI_EXP_LNKCTL, val);
	}
			...
			if (enable) {
				if (rk_pcie->l1ss_ctl1) {
					pci_read_config_dword(
						pdev,
						pdev->l1ss + PCI_L1SS_CTL1,
						&val);
					val &= ~PCI_L1SS_CTL1_L1SS_MASK;
					val |= (rk_pcie->l1ss_ctl1 &
...
static int __maybe_unused rockchip_dw_pcie_suspend(struct device *dev)
{
	...
	ret = readl_poll_timeout(
		rk_pcie->apb_base + PCIE_CLIENT_INTR_STATUS_MSG_RX, status,
		status & BIT(9), 20, RK_PCIE_L2_TMOUT_US);
	if (ret) {
	...
	ret = readl_poll_timeout(rk_pcie->apb_base + PCIE_CLIENT_POWER, status,
				 status & READY_ENTER_L23, 20,
				 RK_PCIE_L2_TMOUT_US);
	if (ret) {
...
no_l2:
	...
	if (ret && ret != -EOPNOTSUPP) {
		dev_err(dev,
			"PHY is reused by other controller, check the dts!\n");
		return ret;
	}
	...
	rk_pcie->intx =
		rk_pcie_readl_apb(rk_pcie, PCIE_CLIENT_INTR_MASK_LEGACY);
